...
infosoftglobal.FusionCharts.prototype = {
	...
	getSWFHTML: function() {
		...
		if (navigator.plugins && navigator.mimeTypes && navigator.mimeTypes.length) { 
			...
			swfNode = '<object id="'+ this.getAttribute('id') +'" codebase="https://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0" '+' classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="'+ this.getAttribute('width') +'" height="'+ this.getAttribute('height') +'">';
			swfNode += '<param name="movie" value="'+ this.getAttribute('swf') +'" />';
			var params = this.getParams();
			//alert(" swf attribute="+ this.getAttribute('swf') +" params =" + params);
			for(var key in params) {
			 swfNode += '<param name="'+ key +'" value="'+ params[key] +'" />';
			}
			var pairs = this.getVariablePairs().join("&");			
			if(pairs.length > 0) {swfNode += '<param name="flashvars" value="'+ pairs +'" />';}
			//CR29075, to show x-shockwave-flash in IE
			swfNode +=' <embed type="application/x-shockwave-flash" src="'+this.getAttribute('swf') +'" width="'+ this.getAttribute('width') +'" height="'+ this.getAttribute('height')+'" ';
			swfNode +=' name="'+ this.getAttribute('id') +'" pluginspage="http://www.macromedia.com/go/getflashplayer"/>';  
			swfNode += "</object>";
		}
		//alert( swfNode);
		return swfNode;
