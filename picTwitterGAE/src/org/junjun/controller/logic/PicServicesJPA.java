...
public class PicServicesJPA implements PicServices{
	...
	public List<Tag> getTag() {
		...
		return null;// mongo.search(null, null, null, null, -1,MAXLIMT , Tag.class);*/
	}
	...
	public User getUser(String id) 
	{
		...
		List<User> items = null;//this.mongo.search(null, null, constrainEQ, null, -1, 1, User.class);
	...
	public List<UIComment> getUIComments(String id) {
		...
		if(id !=null)
		{
			...
			List<Comment> comments = null;//this.mongo.search(null, null, constrainEQ, "date",DAOMongo.DESCENDING, COMMENTLIMIT, Comment.class);
			if(comments!=null)
			{
	...
	public List<Item> getItem(String type, Date dategt,Date datelt, int rating, String sortField, int sortWay,int limit) {
		...
		return null;// mongo.search(constrainLT,constrainGT , constrainEQ, sortField, sortWay, limit, Item.class);
	}
	...
	public Item getItem(String id) {
		Map<String,String> constrainEQ = new  HashMap<String,String>();
		constrainEQ.put("id", id);
		List<Item> items = null;// this.mongo.search(null, null, constrainEQ, null, -1, 1, Item.class);
	...
	public List<Item> getItemByTag(String tag) {
		if(!Buffer.containTag(tag))
			tag = null;		
		List<Item> ret = null;// this.getItem(tag, null, null, -1, "date", this.mongo.DESCENDING, Buffer.BUFFERLIMIT*3);
		return ret;
	}
	...
	public List<Item> getItemByTagRest(String tag, String id) {
		...
		if(tag != null)
			tag = item.getType();
		List<Item> ret =null;//  this.getItem(tag,null, item.getDate(), -1, "date", this.mongo.DESCENDING, Buffer.BUFFERLIMIT);
	    return ret;
	}
	...
	public List<Item> getItemByTagAndKind(String tag, String kind) {
		...
		List<Item> ret = null;// this.getItem(tag, date, null, -1, "total", this.mongo.DESCENDING, Buffer.BUFFERLIMIT*3);
		return ret;
	}
	...
	public Item getNextItem(String id) 
	{
		...
		List<Item> ret = null;// this.getItem(item.getType(),null, item.getDate(), -1, "date", this.mongo.DESCENDING, 1);
		if(ret == null || ret.size() == 0)
		{
			ret = null;// this.getItem(item.getType(), null,new Date(), -1, "date", this.mongo.DESCENDING, 1);
		}
	...
	public Item getPreItem(String id) {
		...
		List<Item> ret = null;// this.getItem(item.getType(),item.getDate(),null,  -1, "date", this.mongo.ASCENDING, 1);
