				...
				{				
					...
					{	
						for(var i = 0; i< data.length;i++)
						{
							...
							$copy.find(".mainimg_a").prop('href',"<%=path%>/${currtag}/"+item.id);
...
<body>
	<span id="kind" class="hiddenid">${sort}</span>
	...
	<div class="box" id="hiddenbox" style="display:none">
		...
		<a href="<%=path%>/${currtag}/${item.id}?sort=${sort}$skip=${vs.count}" class="mainimg_a"><img width=200  class="mainimg" src="${item.url}"  /></a>
		...
		<div class="column" id="column0">
			<c:forEach items="${items}" varStatus="vs" var="item" begin="0" step="4">  
				<div class="box">
					<span class="hiddenid">${item.id}</span>
					<a href="<%=path%>/${currtag}/${item.id}?sort=${sort}$skip=${0+(vs.count-1)*4}" class="mainimg_a"><img  width=200 class="mainimg" src="${item.url}"></a>
					<div class="comment" itemid = "${item.id}">
					  <a class="img_background good"><span>${item.good}  </span></a> 
					  <a class="img_background bad"><span>${item.bad} </span></a>  
					 <%--  <a class="img_background collect"><span>${item.collect} </span></a>   --%>
		...
		<div class="column" id="column1">
			<c:forEach items="${items}" varStatus="vs" var="item" begin="1" step="4">  
				<div class="box">
					<span class="hiddenid">${item.id}</span>
					<a href="<%=path%>/${currtag}/${item.id}?sort=${sort}$skip=${1+(vs.count-1)*4}" class="mainimg_a"><img  width=200 class="mainimg" src="${item.url}"  /></a>
					<div class="comment"  itemid = "${item.id}">
					  <a class="img_background good""><span>${item.good}  </span></a> 
					  <a class="img_background bad"><span>${item.bad} </span></a>  
					 <%--  <a class="img_background collect"><span>${item.collect} </span></a>   --%>
		...
		<div class="column" id="column2">
			<c:forEach items="${items}" varStatus="vs" var="item" begin="2" step="4">  
				<div class="box">
					<span class="hiddenid">${item.id}</span>
					<a href="<%=path%>/${currtag}/${item.id}?sort=${sort}$skip=${2+(vs.count-1)*4}" class="mainimg_a"><img  width=200 class="mainimg" src="${item.url}"></a>
					<div class="comment"  itemid = "${item.id}">
					  <a class="img_background good"><span>${item.good}  </span></a> 
					  <a class="img_background bad"><span>${item.bad} </span></a>  
					 <%--  <a class="img_background collect"><span>${item.collect} </span></a>   --%>
		...
		<div class="column" id="column3">
			<c:forEach items="${items}" varStatus="vs" var="item" begin="3" step="4">  
				<div class="box">
					<span class="hiddenid">${item.id}</span>
					<a href="<%=path%>/${currtag}/${item.id}?sort=${sort}$skip=${3+(vs.count-1)*4}" class="mainimg_a"><img width=200 class="mainimg" src="${item.url}"></a>
					<div class="comment"  itemid = "${item.id}">
					  <a class="img_background good"><span>${item.good}  </span></a> 
					  <a class="img_background bad"><span>${item.bad} </span></a>  
					<%--   <a class="img_background collect"><span>${item.collect} </span></a>   --%>
